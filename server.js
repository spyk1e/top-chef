...
app.get('/GetMichelin', function (req, res) {
    console.log("start scraping on Michelin");
        ...
        LafourchetteLoop(req, res, MichelinList, function (nbRestChecked) {
            ...
            if (listJsonLafourchette.length >= 230) {
                setTimeout(function () {
                    ...
                    console.log("Clean datas manually if needed");
...
function GetLafourchetteJSONInfos(callback) {
    fs.readFile('LafourchetteCleaned.json', 'utf8', function (err, data) { //Use the cleaned data from LF and michelin
        if (err) throw err;
    ...
    for (var iMenu in lafourchettePromoJSON) {
        if (lafourchettePromoJSON[iMenu].is_special_offer == true) {
            ...
            var lafourchettePromo = {
                ...
                promo: "true",
                url_LF: "https://www.lafourchette.com/recherche/_/415144?idRestaurant=" + restaurant.id,
                url_map: "https://www.google.com/maps/search/?api=1&query=" + restaurant.title.replace(/ /g, "%20") + "+" + restaurant.city,
...
app.get('/GetPromo', function (req, res) {
    ...
    setTimeout(function () {
        ...
        WritePromo(function () {
            ...
            res.status(301).redirect('/top-chef');
        })
    }, 10000); // Adjust delay if it's too short
});

//DISPLAY
function DisplayPretty(req, res, callback) {


    fs.readFile('Promo.json', 'utf8', function (err, data) {
        if (err) throw err;

        var listDisplay = [];
        for (var restaurant in JSON.parse(data)) {
            listDisplay.push(JSON.parse(data)[restaurant]);
        }

        callback(listDisplay);
    });
}
...
app.get('/top-chef', (req, res) => {
    ...
    DisplayPretty(req, res, function (listDisplay) {


        //console.log(listDisplay);

        res.send(listDisplay);
        if (!reactOpen) {
        ...
        //res.sendFile(path.join(__dirname + '/client/src/top-chef.html'));
    })
